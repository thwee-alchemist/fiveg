...
/*
 ...
 var fiveg = new FIVEG(); // instantiation
 fiveg.init('#selector', {width: 600, height: 350}); // initialization
 
 var vertex_options = {
   width: 5,  //
   height: 5, // or: size: int
   depth: 5,  // 
 ...
 var vertex1 = fiveg.graph.add_vertex(vertex_options); // add a vertex
 var vertex2 = fiveg.graph.add_vertex(vertex_options); // add another vertex
 
 var edge_options = {}; // currently not defined, but this is how you would pass them.
 
 var edge = fiveg.graph.add_edge(vertex1, vertex2, edge_options);
 
 fiveg.graph.remove_edge(edge); // this is why you keep those variables
 fiveg.graph.remove_vertex(vertex1);
 fiveg.graph.remove_vertex(vertex2);
